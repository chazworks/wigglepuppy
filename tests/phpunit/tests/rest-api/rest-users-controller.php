...
class WP_Test_REST_Users_Controller extends WP_Test_REST_Controller_Testcase
{
    ...
    public static function wpSetUpBeforeClass(WP_UnitTest_Factory $factory)
    {
        ...
        foreach ([ true, false ] as $show_in_rest) {
    ...
    public function test_get_items_with_edit_context($method)
    {
        ...
        if ('HEAD' === $method) {
            $this->assertSame([], $response->get_data(), 'Expected null response data for HEAD request, but received non-null data.');
            return null;
        }
    ...
    public function test_get_items_unauthenticated_includes_authors_of_post_types_shown_in_rest()
    {
        ...
        $rest_post_types = array_values(get_post_types([ 'show_in_rest' => true ], 'names'));
    ...
    public function test_get_items_orderby_name()
    {
        ...
        $low_id  = self::factory()->user->create([ 'display_name' => 'AAAAA' ]);
        $mid_id  = self::factory()->user->create([ 'display_name' => 'NNNNN' ]);
        $high_id = self::factory()->user->create([ 'display_name' => 'ZZZZ' ]);
    ...
    public function test_get_items_orderby_url()
    {
        ...
        $low_id  = self::factory()->user->create([ 'user_url' => 'http://a.com' ]);
        $high_id = self::factory()->user->create([ 'user_url' => 'http://b.com' ]);

        $request = new WP_REST_Request('GET', '/wp/v2/users');
        $request->set_param('orderby', 'url');
        $request->set_param('order', 'desc');
        $request->set_param('per_page', 1);
        $request->set_param('include', [ $low_id, $high_id ]);
        $response = rest_get_server()->dispatch($request);
        $data     = $response->get_data();
        $this->assertSame($high_id, $data[0]['id']);
    ...
    public function test_get_items_orderby_slug()
    {
        ...
        $high_id = self::factory()->user->create([ 'user_nicename' => 'blogin' ]);
        $low_id  = self::factory()->user->create([ 'user_nicename' => 'alogin' ]);
    ...
    public function test_get_items_orderby_slugs()
    {
        ...
        self::factory()->user->create([ 'user_nicename' => 'burrito' ]);
        self::factory()->user->create([ 'user_nicename' => 'taco' ]);
        self::factory()->user->create([ 'user_nicename' => 'chalupa' ]);

        $request = new WP_REST_Request('GET', '/wp/v2/users');
        $request->set_param('orderby', 'include_slugs');
        $request->set_param('slug', [ 'taco', 'burrito', 'chalupa' ]);
        $response = rest_get_server()->dispatch($request);
        $data     = $response->get_data();
    ...
    public function test_get_items_orderby_email()
    {
        ...
        $high_id = self::factory()->user->create([ 'user_email' => 'bemail@gmail.com' ]);
        $low_id  = self::factory()->user->create([ 'user_email' => 'aemail@gmail.com' ]);
    ...
    public function test_get_items_include_query()
    {
        ...
        // 'orderby' => 'asc'.
        $request->set_param('include', [ $id2, $id1 ]);
        $response = rest_get_server()->dispatch($request);
        $data     = $response->get_data();
    ...
    public function test_get_items_search()
    {
        ...
        $yolo_id = self::factory()->user->create([ 'display_name' => 'yololololo' ]);
    ...
    public function test_get_items_search_fields()
    {
        ...
        $yolo_id = self::factory()->user->create([ 'user_email' => 'yololololo@example.localhost' ]);
    ...
    public function test_get_items_slug_array_query()
    {
        ...
        $this->assertSame([ 'burrito', 'enchilada', 'taco' ], $slugs);
    }
    ...
    public function test_get_items_slug_csv_query()
    {
        ...
        $data  = $response->get_data();
        $slugs = wp_list_pluck($data, 'slug');
        $this->assertSame([ 'taco', 'enchilada', 'burrito' ], $slugs);
    }
    ...
    public function test_create_item_illegal_username()
    {
        ...
        add_filter('illegal_user_logins', [ $this, 'get_illegal_user_logins' ]);
        ...
        remove_filter('illegal_user_logins', [ $this, 'get_illegal_user_logins' ]);
    ...
    public function test_create_existing_network_user_on_sub_site_has_error()
    {
        ...
        foreach ($errors as $error) {
            ...
            $this->assertContains($error['code'], [ 'user_name', 'user_email' ]);
    ...
    public function test_update_user_multiple_roles()
    {
        ...
        $this->assertSame([ 'author', 'editor' ], $new_data['roles']);
    ...
    public function test_update_user_role_invalid_privilege_escalation()
    {
        ...
        $request = new WP_REST_Request('PUT', sprintf('/wp/v2/users/%d', self::$editor));
        $request->set_param('roles', [ 'administrator' ]);
        $response = rest_get_server()->dispatch($request);
    ...
    public function test_update_user_role_invalid_role()
    {
        ...
        $request = new WP_REST_Request('PUT', sprintf('/wp/v2/users/%d', self::$editor));
        $request->set_param('roles', [ 'BeSharp' ]);
        $response = rest_get_server()->dispatch($request);
    ...
    public function verify_user_roundtrip($input = [], $expected_output = [])
    {
        if (isset($input['id'])) {
    ...
    {
        $user_id = self::factory()->user->create([ 'display_name' => 'Deleted User' ]);
    ...
    public function test_get_additional_field_registration()
    {
        ...
        if (is_multisite()) {
            ...
            update_site_option('site_admins', [ $current_user->user_login ]);
        }
    ...
    public function test_update_item_from_different_site_as_site_administrator()
    {
        ...
        $request = new WP_REST_Request('PUT', sprintf('/wp/v2/users/%d', $user_id));
        $request->add_header('Content-Type', 'application/x-www-form-urlencoded');
        $request->set_body_params([ 'first_name' => 'New Name' ]);
        $response = rest_get_server()->dispatch($request);
        $this->assertErrorResponse('rest_user_invalid_id', $response, 404);
    }
    ...
    public function test_get_items_only_fetches_ids_for_head_requests($method)
    {
        ...
        add_filter('pre_user_query', [ $filter, 'filter' ], 10, 2);
    ...
    {
        return [
            'get_item request'  => [ '/wp/v2/users/%d' ],
            'get_items request' => [ '/wp/v2/users' ],
        ];
    }
