...
class Tests_REST_Request extends WP_UnitTestCase
{
    ...
    public function data_is_method_should_detect_method_ignoring_case()
    {
        return [
            // GET.
            'GET same case'          => [ 'GET', 'GET', true ],
            'GET different case'     => [ 'GET', 'get', true ],
            'GET different case #2'  => [ 'GET', 'get', true ],
            'GET different case #3'  => [ 'GET', 'gEt', true ],
            'GET wrong method'       => [ 'GET', 'POST', false ],
            // POST.
            'POST same case'         => [ 'POST', 'POST', true ],
            'POST different case'    => [ 'POST', 'post', true ],
            'POST different case #2' => [ 'POST', 'pOsT', true ],
            'POST wrong method'      => [ 'POST', 'GET', false ],
            // HEAD.
            'HEAD same case'         => [ 'HEAD', 'HEAD', true ],
            'HEAD different case'    => [ 'HEAD', 'head', true ],
            'HEAD different case #2' => [ 'HEAD', 'HeAd', true ],
            'HEAD wrong method'      => [ 'HEAD', 'GET', false ],
        ];
    }
