...
class WP_Test_REST_Posts_Controller extends WP_Test_REST_Post_Type_Controller_Testcase
{
    ...
    public static function wpSetUpBeforeClass(WP_UnitTest_Factory $factory)
    {
        ...
        self::$terms   = $factory->term->create_many(15, [ 'taxonomy' => 'category' ]);
        ...
        if (is_multisite()) {
            update_site_option('site_admins', [ 'superadmin' ]);
        }

        // Only support 'post' and 'gallery'.
        self::$supported_formats = get_theme_support('post-formats');
        add_theme_support('post-formats', [ 'post', 'gallery' ]);
    ...
    public function set_up()
    {
        ...
        add_filter('rest_pre_dispatch', [ $this, 'wpSetUpBeforeRequest' ], 10, 3);
        add_filter('posts_clauses', [ $this, 'save_posts_clauses' ], 10, 2);
    }
    ...
    public function test_registered_get_item_params()
    {
        ...
        $data     = $response->get_data();
        $keys     = array_keys($data['endpoints'][0]['args']);
        $this->assertEqualSets([ 'context', 'id', 'password', 'excerpt_length' ], $keys);
    }
    ...
    public function test_get_items_empty_query($method)
    {
        ...
        if ($request->is_method('HEAD')) {
            $this->assertSame([], $response->get_data(), 'Failed asserting that response data is null for HEAD request.');
        } else {
            $this->assertSame([], $response->get_data(), 'Failed asserting that response data is an empty array for GET request.');
        }
    ...
    {
        self::factory()->post->create([ 'post_author' => self::$editor_id ]);
        self::factory()->post->create([ 'post_author' => self::$author_id ]);
        ...
        $request = new WP_REST_Request($method, '/wp/v2/posts');
        $request->set_param('author', [ self::$editor_id, self::$author_id ]);
        $response = rest_get_server()->dispatch($request);
        $this->assertSame(200, $response->get_status());
        $data = $response->get_data();
        if ($request->is_method('get')) {
            $this->assertCount(2, $data);
            $this->assertSameSets([ self::$editor_id, self::$author_id ], wp_list_pluck($data, 'author'));
        } else {
            $this->assertSame([], $data, 'Failed asserting that response data is null for HEAD request.');
            $headers = $response->get_headers();
    ...
    public function test_get_items_author_exclude_query($method)
    {
        ...
        $request = new WP_REST_Request($method, '/wp/v2/posts');
        $request->set_param('per_page', self::$per_page);
        $request->set_param('author_exclude', [ self::$editor_id, self::$author_id ]);
        $response = rest_get_server()->dispatch($request);
        $this->assertSame(200, $response->get_status());
        $data = $response->get_data();
        if ($request->is_method('get')) {
    ...
    public function test_get_items_orderby_author_query()
    {
        ...
        $request = new WP_REST_Request('GET', '/wp/v2/posts');
        $request->set_param('include', [ $id1, $id2, $id3 ]);
    ...
    public function test_get_items_multiple_slugs_array_query()
    {
        ...
        $request = new WP_REST_Request('GET', '/wp/v2/posts');
        $request->set_param('slug', [ 'banana', 'peach' ]);
        $response = rest_get_server()->dispatch($request);
        $this->assertSame(200, $response->get_status());
        $data = $response->get_data();
        $this->assertCount(2, $data);
        ...
        sort($titles);
        $this->assertSame([ 'Banana', 'Peach' ], $titles);
    }
    ...
    public function test_get_items_multiple_slugs_string_query()
    {
        ...
        sort($titles);
        $this->assertSame([ 'Apple', 'Banana' ], $titles);
    }
    ...
    public function test_get_items_multiple_statuses_string_query()
    {
        ...
        sort($statuses);
        $this->assertSame([ 'draft', 'private' ], $statuses);
    }
    ...
    public function test_get_items_multiple_statuses_array_query()
    {
        ...
        self::factory()->post->create([ 'post_status' => 'draft' ]);
        self::factory()->post->create([ 'post_status' => 'pending' ]);
        self::factory()->post->create([ 'post_status' => 'publish' ]);

        $request = new WP_REST_Request('GET', '/wp/v2/posts');
        $request->set_param('context', 'edit');
        $request->set_param('status', [ 'draft', 'pending' ]);
        ...
        sort($statuses);
        $this->assertSame([ 'draft', 'pending' ], $statuses);
    }
    ...
    public function test_get_items_multiple_statuses_one_invalid_query()
    {
        $request = new WP_REST_Request('GET', '/wp/v2/posts');
        $request->set_param('context', 'edit');
        $request->set_param('status', [ 'draft', 'nonsense' ]);
        $response = rest_get_server()->dispatch($request);
        $this->assertErrorResponse('rest_invalid_param', $response, 400);
    }
    ...
    public function test_get_items_multiple_statuses_custom_role_one_invalid_query()
    {
        ...
        $request = new WP_REST_Request('GET', '/wp/v2/posts');
        $request->set_param('status', [ 'private', 'future' ]);
    ...
    {
        return [
            'general search, one exact match and one partial match' => [
                'search_term'  => 'Rye',
                'exact_search' => false,
                'expected'     => 2,
            ],
            'exact search, one exact match and one partial match' => [
                'search_term'  => 'Rye',
                'exact_search' => true,
                'expected'     => 1,
            ],
            'exact search, no match and one partial match' => [
    ...
    public function test_get_items_with_orderby_slugs()
    {
        ...
        $request->set_param('slug', [ 'taco', 'chalupa', 'burrito' ]);
    ...
    public function test_get_items_tags_query()
    {
        ...
        wp_set_object_terms($id1, [ $tag['term_id'] ], 'post_tag');

        $request = new WP_REST_Request('GET', '/wp/v2/posts');
        $request->set_param('tags', [ $tag['term_id'] ]);
    ...
    public function test_get_items_tags_exclude_query()
    {
        ...
        $id2 = self::factory()->post->create([ 'post_status' => 'publish' ]);
        $id3 = self::factory()->post->create([ 'post_status' => 'publish' ]);
        $id4 = self::factory()->post->create([ 'post_status' => 'publish' ]);
        $tag = wp_insert_term('My Tag', 'post_tag');
        ...
        $request = new WP_REST_Request('GET', '/wp/v2/posts');
        $request->set_param('per_page', self::$per_page);
        $request->set_param('tags_exclude', [ $tag['term_id'] ]);
    ...
    public function test_get_items_tags_and_categories_query()
    {
        ...
        wp_set_object_terms($id1, [ $tag['term_id'] ], 'post_tag');
        wp_set_object_terms($id2, [ $tag['term_id'] ], 'post_tag');
        wp_set_object_terms($id1, [ $category['term_id'] ], 'category');
    ...
    public function test_get_items_tags_or_categories_query()
    {
        ...
        wp_set_object_terms($id1, [ $tag['term_id'] ], 'post_tag');
        wp_set_object_terms($id2, [ $category['term_id'] ], 'category');
    ...
    public function test_get_items_tags_and_categories_exclude_query()
    {
        ...
        $request = new WP_REST_Request('GET', '/wp/v2/posts');
        $request->set_param('tags', [ $tag['term_id'] ]);
        $request->set_param('categories_exclude', [ $category['term_id'] ]);
        ...
        $request->set_param('tags_exclude', [ 'my-tag' ]);
        $response = rest_get_server()->dispatch($request);
        $this->assertErrorResponse('rest_invalid_param', $response, 400);
    }
    ...
    public function test_get_items_tags_or_categories_exclude_query()
    {
        ...
        wp_set_object_terms($id2, [ $tag['term_id'] ], 'post_tag');
        wp_set_object_terms($id2, [ $category['term_id'] ], 'category');
        wp_set_object_terms($id3, [ $category['term_id'] ], 'category');
    ...
    public function test_get_items_with_category_including_children()
    {
        ...
        $cat1 = static::factory()->term->create([ 'taxonomy' => $taxonomy->name ]);
    ...
    {
        $taxonomy   = get_taxonomy('category');
        $categories = static::factory()->term->create_many(2, [ 'taxonomy' => $taxonomy->name ]);
    ...
    public function test_get_items_sticky()
    {
        ...
        update_option('sticky_posts', [ $id2 ]);
    ...
    public function test_get_items_sticky_with_include()
    {
        ...
        update_option('sticky_posts', [ $id1, $id2 ]);
    ...
    public function test_get_items_with_custom_search_columns()
    {
        ...
        $request->set_param('search_columns', [ 'post_title' ]);
        $response = rest_get_server()->dispatch($request);
    ...
    public function test_get_items_primes_thumbnail_cache_for_featured_media()
    {
        ...
        $filter = new MockAction();
        add_filter('update_post_metadata_cache', [ $filter, 'filter' ], 10, 2);
    ...
    {
        $post1 = self::factory()->post->create([ 'post_date' => '2016-01-15T00:00:00Z' ]);
        $post2 = self::factory()->post->create([ 'post_date' => '2016-01-16T00:00:00Z' ]);
        $post3 = self::factory()->post->create([ 'post_date' => '2016-01-17T00:00:00Z' ]);
    ...
    {
        $post1 = self::factory()->post->create([ 'post_date' => '2016-01-01 00:00:00' ]);
        $post2 = self::factory()->post->create([ 'post_date' => '2016-01-02 00:00:00' ]);
        $post3 = self::factory()->post->create([ 'post_date' => '2016-01-03 00:00:00' ]);
    ...
    {
        return [
            'get_item request'  => [ '/wp/v2/posts/%d' ],
            'get_items request' => [ '/wp/v2/posts' ],
        ];
    }
    ...
    public function test_get_item_permissions_check_while_updating_password()
    {
        ...
        $request->set_url_params([ 'id' => self::$post_id ]);
    ...
    {
        register_post_status('testpubstatus', [ 'public' => true ]);
        register_post_status('testprivtatus', [ 'public' => false ]);
...
Shankle pork chop prosciutto ribeye ham hock pastrami. T-bone shank brisket bacon pork chop. Cupim hamburger pork loin short loin. Boudin ball tip cupim ground round ham shoulder. Sausage rump cow tongue bresaola pork pancetta biltong tail chicken turkey hamburger. Kevin flank pork loin salami biltong. Alcatra landjaeger pastrami andouille kielbasa ham tenderloin drumstick sausage turducken tongue corned beef.',
    ...
    public function test_create_item_with_template()
    {
        ...
        add_filter('theme_post_templates', [ $this, 'filter_theme_post_templates' ]);
        ...
        remove_filter('theme_post_templates', [ $this, 'filter_theme_post_templates' ]);
    ...
    public function test_create_post_with_categories()
    {
        ...
        $data = $response->get_data();
        $this->assertSame([ $category['term_id'] ], $data['categories']);
    }
    ...
    public function test_create_post_with_categories_as_csv()
    {
        ...
        $data = $response->get_data();
        $this->assertSame([ $category['term_id'], $category2['term_id'] ], $data['categories']);
    }
    ...
    public function test_create_post_with_categories_that_cannot_be_assigned_by_current_user()
    {
        ...
        add_filter('map_meta_cap', [ $this, 'revoke_assign_term' ], 10, 4);
        $response = rest_get_server()->dispatch($request);
        remove_filter('map_meta_cap', [ $this, 'revoke_assign_term' ], 10, 4);
    ...
    public function test_update_post_with_categories()
    {
        ...
        $response = rest_get_server()->dispatch($request);
        $new_data = $response->get_data();
        $this->assertSame([ $category['term_id'] ], $new_data['categories']);
        $categories_path = '';
        $links           = $response->get_links();
    ...
    public function test_update_post_with_empty_categories()
    {
        ...
        $response = rest_get_server()->dispatch($request);
        $new_data = $response->get_data();
        $this->assertSame([], $new_data['categories']);
    }
    ...
    public function verify_post_roundtrip($input = [], $expected_output = [])
    {
        // Create the post.
        $request = new WP_REST_Request('POST', '/wp/v2/posts');
    ...
    {
        $post_id = self::factory()->post->create([ 'post_title' => 'Deleted post' ]);
    ...
    public function test_get_item_schema_issues_doing_it_wrong_when_taxonomy_name_is_already_set_in_properties()
    {
        ...
        register_taxonomy('status', 'post', [ 'show_in_rest' => true ]);
    ...
    public function additional_field_update_callback($value, $post, $field_name)
    {
        if ('returnError' === $value) {
            return new WP_Error('rest_invalid_param', 'Testing an error.', [ 'status' => 400 ]);
        }
    ...
    public function test_sticky_action_ldo_registered_for_posts()
    {
        ...
        $this->assertArrayHasKey('links', $schema);
        $publish = wp_list_filter($schema['links'], [ 'rel' => 'https://api.w.org/action-sticky' ]);
    ...
    public function test_author_action_ldo_registered_for_post_types_with_author_support()
    {
        ...
        $this->assertArrayHasKey('links', $schema);
        $publish = wp_list_filter($schema['links'], [ 'rel' => 'https://api.w.org/action-assign-author' ]);
    ...
    public function test_action_links_only_available_in_edit_context()
    {
        ...
        $post = self::factory()->post->create([ 'post_author' => self::$author_id ]);
    ...
    public function test_publish_action_link_does_not_exist_for_contributor()
    {
        ...
        $post = self::factory()->post->create([ 'post_author' => self::$contributor_id ]);
        $this->assertGreaterThan(0, $post);
    ...
    public function test_putting_same_publish_date_does_not_remove_floating_date()
    {
        ...
        $get->set_query_params([ 'context' => 'edit' ]);
    ...
    {
        add_filter('rest_post_item_schema', [ $this, 'filter_post_item_schema' ]);
        ...
        $this->assertSame([ 'new_context' ], $properties['new_prop']['context']);
    }
    ...
    {
        add_filter('rest_post_item_schema', [ $this, 'filter_post_item_schema_add_property' ]);
    ...
    {
        wp_set_current_user(self::$editor_id);
        self::factory()->post->create([ 'post_name' => 'sample-slug' ]);
    ...
    public function test_standard_post_format_support()
    {
        ...
        $request = new WP_REST_Request('GET', '/wp/v2/posts');
        $request->set_param('format', [ 'standard' ]);
    ...
    public function test_post_format_support()
    {
        ...
        $request = new WP_REST_Request('GET', '/wp/v2/posts');
        $request->set_param('format', [ 'aside' ]);

        $response_aside = rest_get_server()->dispatch($request);

        $request->set_param('format', [ 'invalid_format' ]);
    ...
    public function test_multiple_post_format_support()
    {
        ...
        $request = new WP_REST_Request('GET', '/wp/v2/posts');
        $request->set_param('format', [ 'aside', 'gallery' ]);
    ...
    public function test_get_posts_ignore_sticky_default_prepends_sticky_posts()
    {
        ...
        update_option('sticky_posts', [ $id1 ]);
    ...
    {
        $schema['properties']['something_entirely_new'] = [
            'description' => __('A new prop added with a the rest_post_item_schema filter.'),
            'type'        => 'string',
            'context'     => [ 'new_context' ],
        ];
