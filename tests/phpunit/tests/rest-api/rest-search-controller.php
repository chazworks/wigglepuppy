...
class WP_Test_REST_Search_Controller extends WP_Test_REST_Controller_Testcase
{
    ...
    {
        $response = $this->do_request_with_params([], 'OPTIONS');
        $data     = $response->get_data();

        $this->assertSame('view', $data['endpoints'][0]['args']['context']['default']);
        $this->assertSame([ 'view', 'embed' ], $data['endpoints'][0]['args']['context']['enum']);
    }
    ...
    public function test_get_items_search_prime_ids()
    {
        ...
        add_filter('query', [ $action, 'filter' ], 10, 2);
    ...
    public function test_custom_search_handler_get_items()
    {
        ...
        $request  = $this->get_request(
            [
                ...
                'per_page' => 10,
                'type'     => 'test',
                'subtype'  => [ WP_REST_Search_Controller::TYPE_ANY ],
            ],
        );
    ...
    {
        $response = $this->do_request_with_params([ 'per_page' => 1 ]);
     ...
     */
    private function do_request_with_params($params = [], $method = 'GET')
    {
     ...
     */
    private function get_request($params = [], $method = 'GET')
    {
