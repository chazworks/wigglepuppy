...
class Tests_Comment_GetPageOfComment extends WP_UnitTestCase
{
    public function test_last_comment()
    {
        ...
        // Page 4.
        $comment_last = self::factory()->comment->create_post_comments($p, 1, [ 'comment_date' => '2013-09-24 00:00:00' ]);
        self::factory()->comment->create_post_comments($p, 1, [ 'comment_date' => '2013-09-23 00:00:00' ]);

        // Page 3.
        self::factory()->comment->create_post_comments($p, 1, [ 'comment_date' => '2013-09-22 00:00:00' ]);
        self::factory()->comment->create_post_comments($p, 1, [ 'comment_date' => '2013-09-21 00:00:00' ]);
        self::factory()->comment->create_post_comments($p, 1, [ 'comment_date' => '2013-09-20 00:00:00' ]);

        // Page 2.
        self::factory()->comment->create_post_comments($p, 1, [ 'comment_date' => '2013-09-19 00:00:00' ]);
        self::factory()->comment->create_post_comments($p, 1, [ 'comment_date' => '2013-09-18 00:00:00' ]);
        self::factory()->comment->create_post_comments($p, 1, [ 'comment_date' => '2013-09-17 00:00:00' ]);

        // Page 1.
        self::factory()->comment->create_post_comments($p, 1, [ 'comment_date' => '2013-09-16 00:00:00' ]);
        self::factory()->comment->create_post_comments($p, 1, [ 'comment_date' => '2013-09-15 00:00:00' ]);
        $comment_first = self::factory()->comment->create_post_comments($p, 1, [ 'comment_date' => '2013-09-14 00:00:00' ]);

        $this->assertSame(4, get_page_of_comment($comment_last[0], [ 'per_page' => 3 ]));
        $this->assertSame(2, get_page_of_comment($comment_last[0], [ 'per_page' => 10 ]));

        $this->assertSame(1, get_page_of_comment($comment_first[0], [ 'per_page' => 3 ]));
        $this->assertSame(1, get_page_of_comment($comment_first[0], [ 'per_page' => 10 ]));
    }
    ...
    public function test_subsequent_calls_should_hit_cache()
    {
        ...
        // Prime cache.
        $page_1 = get_page_of_comment($c, [ 'per_page' => 3 ]);

        $num_queries = get_num_queries();
        $page_2      = get_page_of_comment($c, [ 'per_page' => 3 ]);
    ...
    public function test_cache_should_be_invalidated_when_older_comment_is_published()
    {
        ...
        $this->assertSame(1, get_page_of_comment($c1, [ 'per_page' => 2 ]));

        wp_set_comment_status($c3, '1');

        $this->assertSame(2, get_page_of_comment($c1, [ 'per_page' => 2 ]));
    }
    ...
    public function test_query_should_be_limited_to_comments_on_the_proper_post()
    {
        ...
        $found_0 = get_page_of_comment($comments_0[0], [ 'per_page' => 2 ]);
        $this->assertSame(3, $found_0);

        $found_1 = get_page_of_comment($comments_1[1], [ 'per_page' => 2 ]);
    ...
    public function test_page_number_when_unapproved_comments_are_included_for_current_commenter()
    {
        ...
        add_filter('wp_get_current_commenter', [ $this, 'get_current_commenter' ]);

        $page     = get_page_of_comment($new_unapproved, [ 'per_page' => 3 ]);
        $comments = get_comments(
            [
                ...
                'post_id'            => $post,
                'status'             => 'approve',
                'include_unapproved' => [ 'foo@bar.test' ],
                'orderby'            => 'comment_date_gmt',
                'order'              => 'ASC',
            ],
        );

        remove_filter('wp_get_current_commenter', [ $this, 'get_current_commenter' ]);
