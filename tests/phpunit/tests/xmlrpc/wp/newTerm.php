...
class Tests_XMLRPC_wp_newTerm extends WP_XMLRPC_UnitTestCase
{
    ...
    {
        $result = $this->myxmlrpcserver->wp_newTerm([ 1, 'username', 'password', [] ]);
    ...
    public function test_invalid_taxonomy()
    {
        ...
        $result = $this->myxmlrpcserver->wp_newTerm([ 1, 'editor', 'editor', [ 'taxonomy' => 'not_existing' ] ]);
        $this->assertIXRError($result);
        $this->assertSame(403, $result->code);
        $this->assertSame(__('Invalid taxonomy.'), $result->message);
    }
    ...
    public function test_incapable_user()
    {
        ...
        $result = $this->myxmlrpcserver->wp_newTerm([ 1, 'subscriber', 'subscriber', [ 'taxonomy' => 'category' ] ]);
        $this->assertIXRError($result);
