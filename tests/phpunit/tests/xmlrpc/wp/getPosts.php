...
class Tests_XMLRPC_wp_getPosts extends WP_XMLRPC_UnitTestCase
{
    ...
    {
        $result = $this->myxmlrpcserver->wp_getPosts([ 1, 'username', 'password' ]);
    ...
    public function test_incapable_user()
    {
        ...
        $result = $this->myxmlrpcserver->wp_getPosts([ 1, 'subscriber', 'subscriber' ]);
        $this->assertIXRError($result);
        $this->assertSame(401, $result->code);

        $filter = [ 'post_type' => 'page' ];
        $result = $this->myxmlrpcserver->wp_getPosts([ 1, 'subscriber', 'subscriber', $filter ]);
        $this->assertIXRError($result);
        $this->assertSame(401, $result->code);
    }
    ...
    public function test_capable_user()
    {
        ...
        $result = $this->myxmlrpcserver->wp_getPosts([ 1, 'editor', 'editor' ]);
    ...
    public function test_invalid_post_type()
    {
        ...
        $filter = [ 'post_type' => 'invalid_post_type_name' ];
        $result = $this->myxmlrpcserver->wp_getPosts([ 1, 'editor', 'editor', $filter ]);
        $this->assertIXRError($result);
    }
    ...
    public function test_fields()
    {
        ...
        $results = $this->myxmlrpcserver->wp_getPosts([ 1, 'editor', 'editor' ]);
        $this->assertNotIXRError($results);
        $expected_fields = [ 'post_id', 'post_title', 'terms', 'custom_fields', 'link' ]; // Subset of expected fields.
    ...
    public function test_search()
    {
        ...
        $post_ids[] = self::factory()->post->create([ 'post_title' => 'First: Hello, World!' ]);
