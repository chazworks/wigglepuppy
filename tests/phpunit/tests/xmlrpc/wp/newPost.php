...
class Tests_XMLRPC_wp_newPost extends WP_XMLRPC_UnitTestCase
{
    ...
    {
        $result = $this->myxmlrpcserver->wp_newPost([ 1, 'username', 'password', [] ]);
    ...
    public function test_incapable_user()
    {
        ...
        $result = $this->myxmlrpcserver->wp_newPost([ 1, 'subscriber', 'subscriber', [] ]);
        $this->assertIXRError($result);
    ...
    public function test_no_content()
    {
        ...
        $result = $this->myxmlrpcserver->wp_newPost([ 1, 'author', 'author', [] ]);
        $this->assertIXRError($result);
    ...
    public function test_basic_content()
    {
        ...
        $result = $this->myxmlrpcserver->wp_newPost([ 1, 'author', 'author', $post ]);
    ...
    public function test_incapable_publish()
    {
        ...
        $result = $this->myxmlrpcserver->wp_newPost([ 1, 'contributor', 'contributor', $post ]);
        $this->assertIXRError($result);
        $this->assertSame(401, $result->code);
    }
    ...
    public function test_capable_private()
    {
        ...
        $result = $this->myxmlrpcserver->wp_newPost([ 1, 'editor', 'editor', $post ]);
        $this->assertNotIXRError($result);
    }
    ...
    public function test_invalid_taxonomy()
    {
        ...
        $result2 = $this->myxmlrpcserver->wp_newPost([ 1, 'editor', 'editor', $post2 ]);
    ...
    public function test_terms()
    {
        ...
        $post   = [
            'post_title' => 'Test',
            'terms'      => [
                'post_tag' => [ $tag2['term_id'], $tag3['term_id'] ],
            ],
        ];
        $result = $this->myxmlrpcserver->wp_newPost([ 1, 'editor', 'editor', $post ]);
        $this->assertNotIXRError($result);

        $post_tags = wp_get_object_terms($result, 'post_tag', [ 'fields' => 'ids' ]);
    ...
    public function test_terms_names()
    {
        ...
        $post_cats = wp_get_object_terms($result, 'category', [ 'fields' => 'ids' ]);
        ...
        $post2   = [
            'post_title'  => 'Test',
            'terms_names' => [
                'category' => [ $cat1_name, $ambiguous_name ],
            ],
        ];
        $result2 = $this->myxmlrpcserver->wp_newPost([ 1, 'editor', 'editor', $post2 ]);
        $this->assertIXRError($result2);
