...
class Tests_XMLRPC_wp_getTerms extends WP_XMLRPC_UnitTestCase
{
    ...
    {
        $result = $this->myxmlrpcserver->wp_getTerms([ 1, 'username', 'password', 'category' ]);
    ...
    public function test_empty_taxonomy()
    {
        ...
        $result = $this->myxmlrpcserver->wp_getTerms([ 1, 'editor', 'editor', '' ]);
        $this->assertIXRError($result);
        $this->assertSame(403, $result->code);
    ...
    public function test_invalid_taxonomy()
    {
        ...
        $result = $this->myxmlrpcserver->wp_getTerms([ 1, 'editor', 'editor', 'not_existing' ]);
        $this->assertIXRError($result);
        $this->assertSame(403, $result->code);
        $this->assertSame(__('Invalid taxonomy.'), $result->message);
    }
    ...
    public function test_incapable_user()
    {
        ...
        $result = $this->myxmlrpcserver->wp_getTerms([ 1, 'subscriber', 'subscriber', 'category' ]);
        $this->assertIXRError($result);
    ...
    public function test_valid_terms()
    {
        ...
        $results = $this->myxmlrpcserver->wp_getTerms([ 1, 'editor', 'editor', 'category' ]);
    ...
    public function test_custom_taxonomy()
    {
        ...
        $results = $this->myxmlrpcserver->wp_getTerms([ 1, 'editor', 'editor', $tax_name ]);
        $this->assertNotIXRError($results);
