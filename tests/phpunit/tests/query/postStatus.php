...
class Tests_Query_PostStatus extends WP_UnitTestCase
{
    ...
    public static function wpSetUpBeforeClass(WP_UnitTest_Factory $factory)
    {
        ...
        register_post_status('privatefoo', [ 'private' => true ]);
    ...
    {
        register_post_status('foo', [ 'exclude_from_search' => true ]);
    ...
    {
        register_post_status('foo', [ 'exclude_from_search' => false ]);
    ...
    {
        register_post_status('foo', [ 'public' => true ]);
        ...
        foreach (get_post_stati([ 'public' => true ]) as $status) {
    ...
    {
        register_post_status('foo', [ 'protected' => true ]);
    ...
    public function test_single_post_with_nonpublic_status_should_not_be_shown_to_logged_out_users()
    {
        ...
        register_post_status('foo_ps', [ 'public' => false ]);
        $p = self::factory()->post->create([ 'post_status' => 'foo_ps' ]);
    ...
    public function test_single_post_with_nonpublic_and_protected_status_should_be_shown_for_user_who_can_edit_others_posts()
    {
        ...
        $this->assertSame([ $p ], wp_list_pluck($q->posts, 'ID'));
    }
    ...
    {
        register_post_type('foo_pt');
        register_post_status('foo_ps', [ 'public' => true ]);
        $p = self::factory()->post->create(
            [
                'post_status' => 'foo_ps',
                'post_author' => self::$editor_user_id,
    ...
    {
        $p1 = self::factory()->post->create([ 'post_status' => 'trash' ]);
    ...
    public function test_non_singular_queries_using_post_type_any_should_respect_post_type_read_private_posts_cap()
    {
        ...
        $this->assertSameSets([ $post_ids['wptests_pt1_p2'], $post_ids['wptests_pt2_p2'] ], wp_list_pluck($q->posts, 'ID'));
        ...
        $this->assertSameSets([ $post_ids['wptests_pt1_p1'], $post_ids['wptests_pt1_p2'], $post_ids['wptests_pt2_p2'] ], wp_list_pluck($q->posts, 'ID'));
    }
