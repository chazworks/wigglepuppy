...
class Tests_Query_ThePost extends WP_UnitTestCase
{
    ...
    {
        self::$author_ids     = $factory->user->create_many(5, [ 'role' => 'author' ]);
        self::$page_parent_id = $factory->post->create([ 'post_type' => 'page' ]);
    ...
    public function data_the_loop_fields()
    {
        return [
            'all fields'                => [ 'all', 2 ],
            'all fields (empty fields)' => [ '', 2 ],
            'post IDs'                  => [ 'ids', 4 ],
            'post ids and parent'       => [ 'id=>parent', 4 ],
        ];
    }
