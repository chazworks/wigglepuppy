...
class Tests_Post_wpInsertPost extends WP_UnitTestCase
{
    ...
    public function set_up()
    {
        ...
        register_post_type(
            'mapped_meta_caps',
            [
                'capability_type' => [ 'mapped_meta_cap', 'mapped_meta_caps' ],
                'map_meta_cap'    => true,
            ],
        );
        ...
        register_post_type(
            'unmapped_meta_caps',
            [
                'capability_type' => [ 'unmapped_meta_cap', 'unmapped_meta_caps' ],
                'map_meta_cap'    => false,
            ],
        );
        ...
        register_post_type(
            'no_admin_caps',
            [
                'capability_type' => [ 'no_admin_cap', 'no_admin_caps' ],
                'map_meta_cap'    => false,
            ],
        );
    }
    ...
    {
        return wp_next_scheduled('publish_future_post', [ 0 => (int) $post_id ]);
    }
    ...
    public function test_get_post_ancestors_within_loop()
    {
        ...
        $this->assertSame([ $parent_id ], get_post_ancestors(0));
    }
    ...
    {
        $post_id = self::factory()->post->create([ 'post_author' => 0 ]);
    ...
    public function test_wp_insert_post_author_null()
    {
        ...
        $post_id = self::factory()->post->create([ 'post_author' => null ]);
    ...
    public function test_updating_a_post_should_not_trash_categories()
    {
        ...
        $assigned_terms = wp_get_object_terms([ $post_id ], [ 'category' ], []);
    ...
    {
        register_post_status('floating', [ 'date_floating' => true ]);
    ...
    {
        register_post_status('not-floating', [ 'date_floating' => false ]);
    ...
    {
        $tag_1 = wp_insert_term('wp_update_post_tag', 'post_tag', [ 'slug' => 'wp_update_post_tag_1' ]);
        $tag_2 = wp_insert_term('wp_update_post_tag', 'post_tag', [ 'slug' => 'wp_update_post_tag_2' ]);
        $tag_3 = wp_insert_term('wp_update_post_tag', 'post_tag', [ 'slug' => 'wp_update_post_tag_3' ]);

        $post_id = self::factory()->post->create(
            [
                'tags_input' => [ $tag_1['term_id'], $tag_2['term_id'] ],
            ],
        );
        ...
        $this->assertSameSets([ $tag_1['term_id'], $tag_2['term_id'] ], $tags);
        ...
        wp_update_post(
            [
                'ID'         => $post->ID,
                'tags_input' => [ $tag_2['term_id'], $tag_3['term_id'] ],
            ],
        );

        $tags = wp_get_post_tags($post->ID, [ 'fields' => 'ids' ]);
        $this->assertSameSets([ $tag_2['term_id'], $tag_3['term_id'] ], $tags);
    }
