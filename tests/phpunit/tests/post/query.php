...
class Tests_Post_Query extends WP_UnitTestCase
{
    ...
    public function test_category__and_var()
    {
        ...
        $posts = $q->query([ 'category__and' => [ $term_id ] ]);
        ...
        $this->assertSame([ $post_id ], wp_list_pluck($posts, 'ID'));

        $posts2 = $q->query([ 'category__and' => [ $term_id, $term_id2 ] ]);
    ...
    public function test_empty_category__in()
    {
        ...
        $q1 = get_posts([ 'category__in' => [ $cat_id ] ]);
        $this->assertNotEmpty($q1);
        $q2 = get_posts([ 'category__in' => [] ]);
        ...
        $q3 = get_posts([ 'tag__in' => [ $tag_id ] ]);
        $this->assertNotEmpty($q3);
        $q4 = get_posts([ 'tag__in' => [] ]);
        $this->assertNotEmpty($q4);

        $q5 = get_posts([ 'tag_slug__in' => [ $slug ] ]);
        $this->assertNotEmpty($q5);
        $q6 = get_posts([ 'tag_slug__in' => [] ]);
    ...
    public function test_the_posts_filter()
    {
        ...
        add_filter('the_posts', [ $this, 'the_posts_filter' ]);
        ...
        foreach ($query->posts as $post) {
            ...
            $this->assertSame([ $post->ID, 'custom data' ], $post->custom_data);
        }

        remove_filter('the_posts', [ $this, 'the_posts_filter' ]);
    }
    ...
    public function test_post__in_ordering()
    {
        ...
        $ordered = [ $post_id2, $post_id4, $post_id3, $post_id1, $post_id5 ];
    ...
    public function test_orderby_post__in_array()
    {
        ...
        $ordered = [ $posts[2], $posts[0], $posts[3] ];
    ...
    public function test_post__in_attachment_ordering()
    {
        ...
        $ordered = [ $att_ids[5], $att_ids[1], $att_ids[4], $att_ids[3], $att_ids[2] ];
    ...
    public function test_post_name__in()
    {
        ...
        $requested = [ $post_ids[0], $post_ids[3] ];
        $q->query(
            [
                'post_name__in' => [ 'woo', 'me' ],
                'fields'        => 'ids',
            ],
        );
        ...
        $requested = [ $post_ids[1], $post_ids[2] ];
        $q->query(
            [
                'post_name__in' => [ 'hoo', 'test' ],
                'fields'        => 'ids',
            ],
        );
        $actual_posts = $q->get_posts();
    ...
    {
        add_filter('posts_pre_query', [ __CLASS__, 'filter_posts_pre_query' ]);
        ...
        remove_filter('posts_pre_query', [ __CLASS__, 'filter_posts_pre_query' ]);

        $this->assertSame($num_queries, get_num_queries());
        $this->assertSame([ 12345 ], $q->posts);
    }
    ...
    public function test_posts_pre_query_filter_should_respect_set_found_posts()
    {
        ...
        add_filter('the_posts', [ $this, 'filter_the_posts' ]);
        add_filter('found_posts', [ $this, 'filter_found_posts' ]);

        $q = new WP_Query([ 'suppress_filters' => false ]);

        remove_filter('posts_request', '__return_empty_string');
        remove_filter('found_posts_query', '__return_empty_string');
        remove_filter('the_posts', [ $this, 'filter_the_posts' ]);
        remove_filter('found_posts', [ $this, 'filter_found_posts' ]);

        $this->assertSame([ $this->post_id ], wp_list_pluck($q->posts, 'ID'));
    ...
    public function test_set_found_posts_fields_ids()
    {
        ...
        $posts = self::factory()->post->create_many(2, [ 'post_type' => 'wptests_pt' ]);
    ...
    public function test_set_found_posts_not_posts_as_an_array($posts, $expected)
    {
        ...
        $method->invoke($q, [ 'no_found_rows' => false ], []);
    ...
    public function test_split_the_query_object_cache()
    {
        ...
        add_filter('split_the_query', [ $filter, 'filter' ]);
