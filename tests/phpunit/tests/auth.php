...
class Tests_Auth extends WP_UnitTestCase
{
    ...
    public const USER_EMAIL = 'test@password.com';
    public const USER_LOGIN = 'password-user';
    ...
    public function test_wp_check_password_supports_hash_with_increased_bcrypt_cost()
    {
        ...
        add_filter('wp_hash_password_options', [ $this, 'reduce_hash_cost' ]);
        $hash = wp_hash_password($password, PASSWORD_BCRYPT);
        remove_filter('wp_hash_password_options', [ $this, 'reduce_hash_cost' ]);
    ...
    public function test_wp_check_password_supports_hash_with_reduced_bcrypt_cost()
    {
        ...
        add_filter('wp_hash_password_options', [ $this, 'increase_hash_cost' ]);
        $hash = wp_hash_password($password, PASSWORD_BCRYPT);
        remove_filter('wp_hash_password_options', [ $this, 'increase_hash_cost' ]);
    ...
    public function data_passwords(): array
    {
        return [
            [ 'a' ],
            [ 'password' ],
            [ str_repeat('a', self::$password_length_limit) ],
        ];
    }
    ...
    public function test_user_password_against_old_users_table_schema(string $password)
    {
        ...
        add_filter('wp_pre_insert_user_data', [ $this, 'mimic_users_schema_prior_to_44' ]);
    ...
    {
        // Create a new app-only password.
        list(, $item) = WP_Application_Passwords::create_new_application_password(self::$user_id, [ 'name' => 'phpunit' ]);
