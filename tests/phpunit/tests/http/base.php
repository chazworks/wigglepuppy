...
abstract class WP_HTTP_UnitTestCase extends WP_UnitTestCase
{
    ...
    public function set_up()
    {
        ...
        foreach ([ 'curl', 'streams', 'fsockopen' ] as $t) {
    ...
    {
        // 5 : 5 & 301.
        $res = $this->wp_remote_request($this->redirection_script . '?code=301&rt=' . 5, [ 'redirection' => 5 ]);
    ...
    {
        // 5 : 5 & 302.
        $res = $this->wp_remote_request($this->redirection_script . '?code=302&rt=' . 5, [ 'redirection' => 5 ]);
    ...
    {
        // 5 > 0 & 301.
        $res = $this->wp_remote_request($this->redirection_script . '?code=301&rt=' . 5, [ 'redirection' => 0 ]);
    ...
    {
        // 5 > 0 & 302.
        $res = $this->wp_remote_request($this->redirection_script . '?code=302&rt=' . 5, [ 'redirection' => 0 ]);
    ...
    {
        // 5 - 5.
        $res = $this->wp_remote_request($this->redirection_script . '?rt=' . 5, [ 'redirection' => 5 ]);
    ...
    public function test_no_head_redirections()
    {
        ...
        $res = $this->wp_remote_request($this->redirection_script . '?code=302&rt=' . 1, [ 'method' => 'HEAD' ]);
    ...
    {
        // 10 > 5.
        $res = $this->wp_remote_request($this->redirection_script . '?rt=' . 10, [ 'redirection' => 5 ]);
    ...
    {
        // 6 > 5 (close edge case).
        $res = $this->wp_remote_request($this->redirection_script . '?rt=' . 6, [ 'redirection' => 5 ]);
    ...
    {
        // 4 < 5 (close edge case).
        $res = $this->wp_remote_request($this->redirection_script . '?rt=' . 4, [ 'redirection' => 5 ]);
    ...
    public function test_send_headers()
    {
        ...
        $res     = $this->wp_remote_request($this->redirection_script . '?header-check', [ 'headers' => $headers ]);
    ...
    public function test_post_redirect_to_method_300($response_code, $method)
    {
        ...
        $res = $this->wp_remote_post(add_query_arg('response_code', $response_code, $url), [ 'timeout' => 30 ]);
    ...
    public function test_https_url_without_ssl_verification()
    {
        ...
        add_filter('http_request_args', [ $this, 'filter_http_request_args' ]);

        $res = $this->wp_remote_head($url, $args);

        remove_filter('http_request_args', [ $this, 'filter_http_request_args' ]);
