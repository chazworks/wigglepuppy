...
class Tests_Formatting_wpTexturize extends WP_UnitTestCase
{
    ...
    {
        add_filter('gettext_with_context', [ $this, 'filter_translate' ], 10, 4);

        $result = wptexturize($input, true);

        remove_filter('gettext_with_context', [ $this, 'filter_translate' ], 10, 4);
    ...
    {
        add_filter('no_texturize_shortcodes', [ $this, 'filter_shortcodes' ], 10, 1);

        $output = $this->assertSame($output, wptexturize($input));

        remove_filter('no_texturize_shortcodes', [ $this, 'filter_shortcodes' ], 10, 1);
        return $output;
    }
...
String with a number followed by a single quote &#8216;Expendables 3&#8217; vestibulum in arcu mi.',
    ...
    {
        add_filter('gettext_with_context', [ $this, 'filter_translate2' ], 10, 4);

        $result = wptexturize($input, true);

        remove_filter('gettext_with_context', [ $this, 'filter_translate2' ], 10, 4);
        wptexturize('reset', true);
