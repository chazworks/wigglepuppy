...
class WP_HTML_Processor extends WP_HTML_Tag_Processor
{
    ...
    private function create_fragment_at_current_node(string $html)
    {
        ...
        if (
            'html' === $namespace &&
            in_array($tag_name, [ 'IFRAME', 'NOEMBED', 'NOFRAMES', 'SCRIPT', 'STYLE', 'TEXTAREA', 'TITLE', 'XMP', 'PLAINTEXT' ], true)
        ) {
            _doing_it_wrong(
                __METHOD__,
                sprintf(
        ...
        $fragment_processor->breadcrumbs = [ 'HTML', $fragment_processor->context_node->node_name ];
     ...
     * @var string
     */
    public const PROCESS_CURRENT_NODE = 'process-current-node';
     ...
     * @access private
     */
    public const CONSTRUCTOR_UNLOCK_CODE = 'Use WP_HTML_Processor::create_fragment() instead of calling the class constructor directly.';
}
