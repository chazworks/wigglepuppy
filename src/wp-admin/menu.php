...
$submenu['upload.php'][10] = [ __('Add Media File'), 'upload_files', 'media-new.php' ];
$i                         = 15;
...
$submenu['link-manager.php'][10] = [ __('Add Link'), 'manage_links', 'link-add.php' ];
...
if (wp_is_block_theme()) {
    ...
    if ($supports_stylebook) {
        $submenu['themes.php'][6] = [ _x('Design', 'design menu item'), 'edit_theme_options', 'site-editor.php' ];
    } else {
        $submenu['themes.php'][6] = [ _x('Patterns', 'patterns menu item'), 'edit_theme_options', 'site-editor.php?p=/pattern' ];
    }
}
...
if (! wp_is_block_theme() || has_action('customize_register')) {
    $submenu['themes.php'][7] = [ __('Customize'), 'customize', esc_url($customize_url), '', 'hide-if-no-customize' ];
}
...
if (! is_multisite()) {
    $submenu['plugins.php'][10] = [ __('Add Plugin'), 'install_plugins', 'plugin-install.php' ];
    if (wp_is_block_theme()) {
...
if (current_user_can('list_users')) {
    ...
    if (current_user_can('create_users')) {
        ...
        $submenu['users.php'][10] = [ __('Add User'), 'promote_users', 'user-new.php' ];
    }
    ...
    if (current_user_can('create_users')) {
        $submenu['profile.php'][10] = [ __('Add User'), 'create_users', 'user-new.php' ];
    } elseif (is_multisite()) {
        $submenu['profile.php'][10] = [ __('Add User'), 'promote_users', 'user-new.php' ];
    }
}
